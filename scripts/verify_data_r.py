...
path_p_r = '/home/adamliu/Desktop/DNN-TTS-V5_MINE/data/source/set/p/cmu_us_arctic_slt_a0001.bin'
...
logging.debug("\np")
with open(path_p_r , mode="rb" ) as f :
    tmp = f.read()
...
with open('log/myphone_log_r.txt','w') as outfile:
    df.to_string(outfile)

'''
logging.debug("\na")
with open(path_a_r , mode="rb" ) as f :
    tmp = f.read()
buffer = struct.unpack( "f" * (len(tmp) // 4) , tmp )
...
logging.debug("\nb")
with open(path_b_r , mode="rb" ) as f :
    tmp = f.read()
buffer = struct.unpack( "f" * (len(tmp) // 4) , tmp )
...
logging.debug("\nc")
with open(path_c_r , mode="rb" ) as f :
    tmp = f.read()
buffer = struct.unpack( "f" * (len(tmp) // 4) , tmp )
...
logging.debug("\nd")
with open(path_d_r , mode="rb" ) as f :
    tmp = f.read()
buffer = struct.unpack( "f" * (len(tmp) // 4) , tmp )
